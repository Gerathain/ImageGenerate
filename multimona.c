...
int mutatedindex;
int othermutatedindex;
shape_t mutatedshape_t;
shape_t othermutatedshape_t;
...
static void * mainloop(void * ptr)
{
    ...
    cairo_surface_t * pngsurf = (cairo_surface_t *) ptr;
    ...
    for(;;)
    {
	if(localbeststep < beststep)
	{
	    if(localbeststep == beststep - 1)
	    {
		if(othermutatedindex == -1)
		{
		    dna_test[mutatedindex] = mutatedshape_t;
		}
		else
		{
		    dna_test[mutatedindex] = mutatedshape_t;
		    dna_test[othermutatedindex] = othermutatedshape_t;
		}
	    }
	    else
	    {
	    ...
	    localbeststep = beststep;
	}
	int other_mutated = mutate(&mutated_shape, dna_test);
	    ...
	    char progress[50];
	    sprintf(progress, "%d", beststep);
	    fwrite(progress, sizeof(progress), 1, f);
	    fclose(f);
	    if(beststep % 100 == 0)
	    {
		char pngName[50];
		sprintf(pngName, "%d.png", beststep / 100);
		cairo_surface_write_to_png(test_surf, pngName);
	    }
	    //printf("%d\n", beststep);
            // test is good, copy to best
            dna_best[mutated_shape] = dna_test[mutated_shape];
	    mutatedshape_t = dna_test[mutated_shape];
	    mutatedindex = mutated_shape;
	    othermutatedindex = -1;
            if(other_mutated >= 0)
	    {
                dna_best[other_mutated] = dna_test[other_mutated];
		othermutatedshape_t = dna_test[other_mutated];
		othermutatedindex = other_mutated;
	    }
            lowestdiff = diff;
        }
        else
...
int main(int argc, char ** argv) {
    ...
    pthread_t thread2;
    pthread_t thread3;
    pthread_create(&thread0, NULL, mainloop, (void *) pngsurf);
    pthread_create(&thread1, NULL, mainloop, (void *) pngsurf);
    pthread_create(&thread2, NULL, mainloop, (void *) pngsurf);
    pthread_create(&thread3, NULL, mainloop, (void *) pngsurf);
    ...
    cairo_surface_write_to_png(best_surf, pngname);
